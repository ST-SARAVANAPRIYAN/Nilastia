...
from nilastia.utils.dots.diff import Changeset
...
from nilastia.utils.dots.packages import PackageError, PackageInstaller
from nilastia.utils.dots.source import DotsSource, SourceError
from nilastia.utils.dots.state import DotsState
from nilastia.utils.io import confirm, disable_input, fatal, info, log, prompt_selection, warn
...
class Command:
    ...
    def run(self) -> None:
        ...
        if not self.args.noconfirm:
            if confirm("Would you like to back up your current configurations before updating?", default=True):
                from nilastia.subcommands.backup import Command as BackupCommand
                BackupCommand(Namespace(action="create", name=None)).run()
        ...
        try:
            ...
            # Restart quickshell service
            subprocess.run(["systemctl", "--user", "restart", "niri-nilastia-shell.service"], check=False)
            info("Nilastia shell restarted successfully.")
        except Exception as e:
            warn(f"Failed to rebuild/restart shell automatically: {e}")
            warn("Please run ./setup_nilastia.sh manually.")

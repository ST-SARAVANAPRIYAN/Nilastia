...
from nilastia.utils.paths import atomic_dump, scheme_data_dir, scheme_path, user_scheme_dir
...
class Scheme:
    ...
    def get_colours_path(self) -> Path:
        user_path = (user_scheme_dir / self.name / self.flavour / self.mode).with_suffix(".txt")
        if user_path.exists():
            return user_path
...
def get_scheme_names() -> list[str]:
    names = set(f.name for f in scheme_data_dir.iterdir() if f.is_dir())
    if user_scheme_dir.exists():
        names.update(f.name for f in user_scheme_dir.iterdir() if f.is_dir())
    return sorted(list(names)) + ["dynamic"]
...
def get_scheme_flavours(name: str | None = None) -> list[str]:
    ...
    flavours = set()
    system_dir = scheme_data_dir / name
    if system_dir.exists():
        flavours.update(f.name for f in system_dir.iterdir() if f.is_dir())
    user_dir = user_scheme_dir / name
    if user_dir.exists():
        flavours.update(f.name for f in user_dir.iterdir() if f.is_dir())
...
def get_scheme_modes(name: str | None = None, flavour: str | None = None) -> list[str]:
    ...
    modes = set()
    system_dir = scheme_data_dir / name / flavour
    if system_dir.exists():
        modes.update(f.stem for f in system_dir.iterdir() if f.is_file())
    user_dir = user_scheme_dir / name / flavour
    if user_dir.exists():
        modes.update(f.stem for f in user_dir.iterdir() if f.is_file())

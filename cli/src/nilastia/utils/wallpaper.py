...
from nilastia.utils.colourfulness import get_variant
from nilastia.utils.hypr import message
from nilastia.utils.material import get_colours_for_image
...
from nilastia.utils.scheme import Scheme, get_scheme
from nilastia.utils.theme import apply_colours


def is_valid_image(path: Path) -> bool:
    return path.is_file() and path.suffix.lower() in [
        ".jpg", ".jpeg", ".png", ".webp", ".tif", ".tiff", ".gif",
        ".mp4", ".webm", ".mkv", ".mov", ".json", ".nilawall"
    ]
...
def set_wallpaper(wall: Path, no_smart: bool) -> None:
    ...
    # Use gif's 1st frame, JSON's first layer, or fallback for videos
    wall_cache = wall
    if wall.suffix.lower() == ".gif":
        wall_cache = convert_gif(wall)
    elif wall.suffix.lower() in [".json", ".nilawall"]:
        try:
            import json
            data = json.loads(wall.read_text())
            if "parallax" in data and "layers" in data["parallax"] and len(data["parallax"]["layers"]) > 0:
                first_img_layer = None
                for layer in data["parallax"]["layers"]:
                    if layer.get("source") and not layer["source"].startswith("virtual://"):
                        first_img_layer = layer
                        break
                
                if first_img_layer:
                    first_layer_src = first_img_layer["source"]
                    if first_layer_src.startswith("data:"):
                        import base64
                        header, encoded = first_layer_src.split(",", 1)
                        ext = ".png"
                        ...
                        temp_file = wallpapers_cache_dir / f"temp_base64_first_layer{ext}"
                        temp_file.parent.mkdir(parents=True, exist_ok=True)
                        temp_file.write_bytes(base64.b64decode(encoded))
                        wall_cache = temp_file
                    else:
                        first_layer_path = (wall.parent / first_layer_src).resolve()
                        if first_layer_path.is_file():
                            wall_cache = first_layer_path
        except Exception:
            pass

...
from nilastia.utils.dots.source import DotsSource
from nilastia.utils.io import warn
from nilastia.utils.paths import wallpapers_dir
from nilastia.utils.scheme import get_scheme_names, scheme_variants
from nilastia.utils.wallpaper import get_wallpaper


def parse_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
    parser = argparse.ArgumentParser(prog="nilastia", description="Main control script for the Caelestia dotfiles")
    ...
    # Create parser for output opts
    output_parser = command_parser.add_parser("output", help="configure niri outputs")
    output_parser.set_defaults(cls=output.Command)
    output_parser.add_argument("name", help="the name of the output/display to configure")
    output_parser.add_argument("-m", "--mode", help="the resolution mode, e.g. 1920x1080@60.001")
    output_parser.add_argument("-s", "--scale", type=float, help="the scaling factor, e.g. 1.0 or 2.0")
    output_parser.add_argument("--vrr", dest="vrr", action="store_const", const=True, help="enable variable refresh rate")
    output_parser.add_argument("--no-vrr", dest="vrr", action="store_const", const=False, help="disable variable refresh rate")
    output_parser.add_argument("--off", dest="status", action="store_const", const="off", help="disable the output")
    output_parser.add_argument("--on", dest="status", action="store_const", const="on", help="enable the output")

    # Create parser for backup opts
    backup_parser = command_parser.add_parser("backup", help="manage configuration backups")
    backup_parser.set_defaults(cls=backup.Command)
    backup_parser.add_argument("action", choices=["create", "list", "restore"], help="backup action to perform")
    backup_parser.add_argument("-n", "--name", help="name of the backup archive to create or restore")

    # Create parser for doctor opts
    doctor_parser = command_parser.add_parser("doctor", help="run system health diagnostics")
    doctor_parser.set_defaults(cls=doctor.Command)

    # Create parser for uninstall opts
    uninstall_parser = command_parser.add_parser("uninstall", help="uninstall the Nilastia dotfiles")
    uninstall_parser.set_defaults(cls=uninstall.Command)

...
void PluginManifest::parse() {
    ...
    if (parseError.isEmpty()) {
        ...
        else if (!satisfiesRequirement(requirement, parseVersion(QStringLiteral(NILASTIA_VERSION))))
            parseError = QStringLiteral("Requires Nilastia %1 (current is %2)")
                             .arg(requirement, QStringLiteral(NILASTIA_VERSION));
    }

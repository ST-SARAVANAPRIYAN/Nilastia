...
#ifndef NILASTIA_VERSION
#define NILASTIA_VERSION ""
#endif

Q_LOGGING_CATEGORY(lcPlugins, "nilastia.plugins", QtInfoMsg)
...
QString configDir() {
    return QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QStringLiteral("/nilastia/");
}

QString pluginDataDir() {
    return QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QStringLiteral("/nilastia/plugins");
}
...
QString Plugins::shellVersion() const {
    return QStringLiteral(NILASTIA_VERSION);
}
...
QStringList Plugins::searchRoots() const {
    ...
    const auto envPath = qEnvironmentVariable("NILASTIA_PLUGIN_PATH");
    if (!envPath.isEmpty())

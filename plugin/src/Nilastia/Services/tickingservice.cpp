...
TickingService::TickingService(QObject* parent)
    ...
    auto* dash = nilastia::config::GlobalConfig::instance()->dashboard();
    applyInterval(dash->resourceUpdateInterval());
    QObject::connect(dash, &nilastia::config::DashboardConfig::resourceUpdateIntervalChanged, this, [this, dash] {
        applyInterval(dash->resourceUpdateInterval());
    });
}

...
Q_LOGGING_CATEGORY(lcAc, "nilastia.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "nilastia.services.ac.worker", QtInfoMsg)
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "nilastia-shell", props, &events, this);
    if (!m_stream) {
